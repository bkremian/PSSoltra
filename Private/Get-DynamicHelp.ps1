...
.PARAMETER COMMAND
    PSSoltra Command name(s)
.PARAMETER EXCLUSIONS
    Soltra endpoint names to exclude from results
#>
    [CmdletBinding()]
    [OutputType()]
    param(
    ...
    process {
        ...
        foreach ($Endpoint in (($ParamDetail.Attributes.ParameterSetName | Group-Object).Name |
            ...
            if ($Soltra.Endpoint($Endpoint).Description) {
                # Add endpoint description
                $HeaderText += "  $($Soltra.Endpoint($Endpoint).Description)"
            }
            if ($Soltra.Endpoint($Endpoint).Permission) {
                # Add permission
                $HeaderText += ("`n    Requires" +
                " $((($Soltra.Endpoint($Endpoint)).Permission))")
            }
            # Output header text
            $HeaderText
